...
// `image` boleh berupa URL video (Media Library) → diputar otomatis (muted, loop).
export const ParallaxHero = ({ image, children, className = "", minH = "min-h-[92vh]" }) => {
  ...
  return (
    <section ref={ref} className={`relative flex ${minH} items-center overflow-hidden ${className}`}>
      <motion.div className="absolute inset-0" style={reduce ? undefined : { y, scale }} aria-hidden="true">
        <MediaBackdrop src={image} testId="hero-backdrop" />
      </motion.div>
